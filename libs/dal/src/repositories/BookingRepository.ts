  ...
  implements IBookingRepository<Booking>
{
  ...
  private async getAllRecurringBookings(userId?: string) {
    const match: any = { recurring: { $exists: true }, canceledAt: null };
  ...
  private getDayOfWeekStr(date: string): string {
    ...
    return ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][jsDayOfWeek];
  }
  ...
  private async getRecurringBookings(date: string, dayOfWeekStr: string, userId?: string) {
    const match: any = { recurring: { $exists: true }, canceledAt: null };
    if (userId) match.userId = userId;
    return this.repository.aggregate([
      { $match: match },
  ...
  async findAllBookingsByDate(date: string): Promise<Array<Booking>> {
    ...
    const recurringBookings = await this.getRecurringBookings(date, dayOfWeekStr);
  ...
  async findUserUpcomingBookings(
    ...
    for (const b of allRecurring) {
      ...
      const end = b.recurring.endDate ? new Date(b.recurring.endDate) : undefined;
      ...
      if (Array.isArray(b.recurring.daysOfWeek)) {
        for (const dayName of b.recurring.daysOfWeek) {
          ...
          while (this.getDayOfWeekStr(current.toISOString().slice(0, 10)) !== dayName) {
    ...
    unique.sort((a, b) => new Date(a.bookingDate).getTime() - new Date(b.bookingDate).getTime());
